#!/usr/bin/env bash
# OPERATOR-ONLY begin-signin wrapper (second step of the two-step flow).
#
# After the dedicated persistent professional browser profile has been
# positioned on Planner Web via scripts/operator_auth_bootstrap_navigate.sh,
# this triggers exactly ONE navigation to the FIXED Microsoft authentication
# host so that an operator can complete an interactive Microsoft sign-in
# (including MFA) by hand.
#
# Hard invariants:
