...
performs **no** chooser click (the click path is only taken when the email control is structurally absent). The
text-only marker path is preserved unchanged when structural detection is unavailable. Ambiguous / multiple email
controls (either key counts to more than one candidate) are NOT treated as a signal: the resolver fails closed
without guessing or clicking. Error surfaces still win over any coincidental structural email control (an error
page is not the surface). The fix is minimal and does NOT add any new action/click/forwardable surface;
`_FORWARDABLE_SURFACES` is unchanged and no GUI/VNC/noVNC, consent/CA/enrollment/method/MFA automation is
introduced.
...
**AUTH-112 — Operator-only submit surface-latch gate.** `operator-submit` (AUTH-101) MAY apply the two memory-only credential fields ONLY after the pre-email sign-in surface has been deterministically resolved to `EMAIL_ENTRY` via the operator-only `POST /auth/bootstrap/resolve-signin-surface` (AUTH-109) route. `begin_auth_signin` (AUTH-096) clears the latch (it may recreate an intermediate surface — account chooser / "use another account" prompt — rather than the email-entry field), and only a successful `resolve_signin_surface` sets it. A direct `operator-submit` after `begin-signin` (with no intermediate resolve, or after a non-EMAIL_ENTRY surface) is refused `503 SIGNIN_SURFACE_NOT_RESOLVED` and types nothing — this is the canonical fix for the email NO_MATCH bug where submit was applied against a non-email-entry surface. The latch carries no identity/DOM/URL.
...
**AUTH-115 — Operator-only value-free structural census probe for unrecognized post-auth surfaces.** When the post-KMSI surface classifies `AMBIGUOUS`/`UNKNOWN` (neither the text-marker classifier in `signin_surface.py` nor the MFA phrase matcher in `auth_flow.py` recognizes it), `GET /auth/bootstrap/diagnose-post-auth-surface` returns a CLOSED, value-free structural census of the live page: role counts (button/link/textbox/…), frame-origin classes (microsoft/neutral/other), fixed Microsoft label presence booleans, document ready-state, and email-entry control presence. It is the read-only twin of `diagnose-signin-surface` (AUTH-109-diagnose): same guard chain (started live browser, dedicated persistent professional profile, approved Microsoft authentication origin, exactly one open auth page), SOCKET-level loopback admission, GET-only, zero-parameter, zero-body, absent from every MCP tool/capability/agent-card catalog, the typed `/operations` dispatcher and the control-plane worker client. It reads NO page text, NO URL, NO DOM value, NO cookie, NO token, NO UPN, NO tenant id, NO account identifier — only ints, bools, and CLOSED-enum members — and NEVER clicks, types, navigates, or acts. AUTH-115 is an investigation primitive: it informs the next deterministic fix (e.g. a new allowlisted surface marker + minimal class-scoped action) but by itself changes no surface resolution and never weakens the attestation, evaluator, loopback, credential, or MFA controls.
...
| AUTH-115 | Operator-only value-free structural census probe for unrecognized post-auth surfaces |
