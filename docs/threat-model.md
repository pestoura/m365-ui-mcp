...
| ID | STRIDE | Threat | Controls | Status | Residual risk |
| --- | --- | --- | --- | --- | --- |
| **THR-030** | Spoofing | Something other than the control plane calls the worker | Internal-only Docker network (`internal: true`), host-published loopback only `127.0.0.1:8090` (`ARCH-035`) | IMPLEMENTED | Any container attached to that network can call it. Request authentication (HMAC, `SEC-070`) is `PLANNED`. **Medium.** |
