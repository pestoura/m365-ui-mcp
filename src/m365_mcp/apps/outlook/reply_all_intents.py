"""Fail-closed synthetic reply-all intent preparation for OUT-052."""
...
def prepare_reply_all_intent(
    ...
    *,
    ...
    """Prepare but never execute a governed synthetic reply-all intent."""
    if not readiness.ready_for_readonly_discovery:
        raise ValueError("Outlook read-only discovery is not ready")
    if not fixture.synthetic:
        raise ValueError("OUT-052 requires synthetic source fixture")
    ...
    return SyntheticOutboundIntent(
        ...
        kind=OutboundIntentKind.REPLY_ALL,
        draft_key=draft_key,
        source_message_key=source_message_key,
    )


__all__ = ["prepare_reply_all_intent"]
