...
class SyntheticDirectoryPerson:
    ...
    def __post_init__(self) -> None:
        if not self.person_key or self.person_key != self.person_key.strip() or "@" in self.person_key:
    ...
    def to_projection(self) -> dict[str, object]:
        return {"person_key": self.person_key, "display_name": self.display_name, "role": self.role, "org_unit": self.org_unit, "manager_key": self.manager_key, "synthetic": True}
...
def default_synthetic_directory() -> tuple[SyntheticDirectoryPerson, ...]:
    return (
        ...
        SyntheticDirectoryPerson("dir-architect", "Alex Architect", "Architect", "Security", "dir-lead"),
        SyntheticDirectoryPerson("dir-engineer", "Evan Engineer", "Engineer", "Platform", "dir-lead"),
    )
...
def _catalog(directory: tuple[SyntheticDirectoryPerson, ...] | None) -> tuple[SyntheticDirectoryPerson, ...]:
...
def search_fixture_directory(fixture: OutlookMockFixture, query: str, *, readiness: OutlookReadinessReport, directory: tuple[SyntheticDirectoryPerson, ...] | None = None) -> tuple[SyntheticDirectoryPerson, ...]:
    ...
    return tuple(item for item in _catalog(directory) if needle in item.display_name.casefold() or needle in item.role.casefold() or needle in item.org_unit.casefold())


def read_fixture_org_context(fixture: OutlookMockFixture, person_key: str, *, readiness: OutlookReadinessReport, directory: tuple[SyntheticDirectoryPerson, ...] | None = None) -> dict[str, object]:
    _gate(fixture, readiness)
    ...
    direct_reports = tuple(sorted(item.person_key for item in catalog if item.manager_key == person_key))
    return {"person": match.to_projection(), "direct_report_keys": direct_reports, "synthetic": True}


__all__ = ["SyntheticDirectoryPerson", "default_synthetic_directory", "read_fixture_org_context", "search_fixture_directory"]
