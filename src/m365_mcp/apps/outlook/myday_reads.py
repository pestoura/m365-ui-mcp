...
class SyntheticSmartTask:
    ...
    def __post_init__(self) -> None:
        ...
        invalid_planned_offset = self.planned_day_offset is not None and (
            isinstance(self.planned_day_offset, bool)
            or not -3650 <= self.planned_day_offset <= 3650
        )
        if invalid_planned_offset:
    ...
    def to_projection(self) -> dict[str, object]:
        return {
            ...
            "planned_day_offset": self.planned_day_offset,
            "my_day": self.my_day,
            "synthetic": True,
        }
...
def default_synthetic_smart_tasks() -> tuple[SyntheticSmartTask, ...]:
    return (
        ...
        SyntheticSmartTask(
            ...
            planned_day_offset=2,
        ),
        SyntheticSmartTask(
            "smart-charlie",
            "Closed synthetic task",
            completed=True,
            planned_day_offset=-1,
        ),
    )
...
def read_fixture_smart_list(
    fixture: OutlookMockFixture,
    kind: SmartListKind,
    *,
    readiness: OutlookReadinessReport,
    reference_day_offset: int = 0,
    tasks: tuple[SyntheticSmartTask, ...] | None = None,
) -> tuple[SyntheticSmartTask, ...]:
    ...
    if kind is SmartListKind.PLANNED:
        return tuple(
            item
            for item in catalog
            if item.planned_day_offset is not None
            and item.planned_day_offset >= reference_day_offset
...
__all__ = [
    "SmartListKind",
    "SyntheticSmartTask",
    "default_synthetic_smart_tasks",
    "read_fixture_smart_list",
]
