...
def _context(valid: bool = True) -> shared_mailbox_context.SharedMailboxContext:
    return shared_mailbox_context.SharedMailboxContext(
        state=(shared_mailbox_context.SharedMailboxContextState.VERIFIED if valid else shared_mailbox_context.SharedMailboxContextState.UNVERIFIED),
        primary_context_verified=True,
...
def _identities() -> tuple[from_identity_mutations.SyntheticFromIdentity, ...]:
    return (
        from_identity_mutations.SyntheticFromIdentity("shared-alpha", from_identity_mutations.FromIdentityMode.SHARED, True),
        from_identity_mutations.SyntheticFromIdentity("delegated-alpha", from_identity_mutations.FromIdentityMode.DELEGATED, True),
    )
...
def test_send_as_fails_closed_for_scope_mode_and_address_shape() -> None:
    with pytest.raises(ValueError, match="verified shared mailbox context"):
        shared_mailbox_send_as.prepare_shared_mailbox_send_as(
            _context(False), draft_models.default_synthetic_drafts(), draft_key="draft-001", identity_key="shared-alpha", readiness=_ready(), identities=_identities()
        )
    with pytest.raises(ValueError, match="SHARED"):
        shared_mailbox_send_as.prepare_shared_mailbox_send_as(
            _context(), draft_models.default_synthetic_drafts(), draft_key="draft-001", identity_key="delegated-alpha", readiness=_ready(), identities=_identities()
        )
    with pytest.raises(ValueError, match="must not encode an address"):
        shared_mailbox_send_as.prepare_shared_mailbox_send_as(
            _context(), draft_models.default_synthetic_drafts(), draft_key="draft-001", identity_key="shared@example.test", readiness=_ready(), identities=_identities()
        )


def test_out116_remains_reserved_and_not_public() -> None:
    assert default_application_registry().get(ApplicationKey.OUTLOOK).state is ApplicationState.RESERVED
