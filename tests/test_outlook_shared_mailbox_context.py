...
def test_unobserved_shared_shell_stays_unverified() -> None:
    result = verify_shared_mailbox_context(
        _verified_primary(),
        SharedMailboxObservation(shared_shell_observed=False),
    )

    assert result.state is SharedMailboxContextState.UNVERIFIED
    assert result.valid is False
...
def test_public_surface_carries_no_identity_parameters() -> None:
    forbidden = ("address", "upn", "tenant", "user", "url", "token", "cookie", "storage")
    names = set(inspect.signature(verify_shared_mailbox_context).parameters)
    names |= set(inspect.signature(SharedMailboxObservation).parameters)

    assert names == {
        "primary_context",
        "observation",
        "reattestation_required",
        "shared_shell_observed",
        "primary_mailbox_indicator",
        "ambiguous_mailbox_context",
        "scope_digest",
        "evidence_digest",
    }
    for name in names:
