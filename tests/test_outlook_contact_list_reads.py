...
def test_list_and_get_contact_lists() -> None:
    ...
    lists = contact_list_reads.list_fixture_contact_lists(fixture, readiness=_ready())
    assert [item.list_key for item in lists] == ["list-security", "list-platform"]
    selected = contact_list_reads.get_fixture_contact_list(fixture, "list-security", readiness=_ready())
...
def test_duplicate_members_and_unknown_list_fail_closed() -> None:
    try:
        contact_list_reads.SyntheticContactList("list-a", "A", ("person-a", "person-a"))
    except ValueError as exc:
        assert "unique" in str(exc)
    else:
        raise AssertionError("duplicate members accepted")
    fixture = mock_ui.default_outlook_fixture()
    try:
        contact_list_reads.get_fixture_contact_list(fixture, "missing", readiness=_ready())
    except ValueError as exc:
...
def test_projection_excludes_identity_and_browser_primitives() -> None:
    fixture = mock_ui.default_outlook_fixture()
    projection = repr(contact_list_reads.get_fixture_contact_list(fixture, "list-security", readiness=_ready()).to_projection()).lower()
